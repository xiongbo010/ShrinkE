...
class DataManager(object):
    ...
    def load(config: Union[dict, FancyDict]) -> Callable:
        ...
        elif config['DATASET'] == 'wikipeople50':
            if config['STATEMENT_LEN'] == 5:
                if config['CLEANED_DATASET']:
                    return partial(load_clean_wikipeople50_statements, subtype="quints")
                else:
                    return load_wikipeople_quints
            elif config['STATEMENT_LEN'] == 3:
                if config['CLEANED_DATASET']:
                    return partial(load_clean_wikipeople50_statements, subtype="triples")
                else:
                    return load_wikipeople_triples50
            else:
                if config['CLEANED_DATASET']:
                    return partial(load_clean_wikipeople50_statements, subtype="statements", maxlen=config['MAX_QPAIRS'])
                else:
                    return partial(load_wikipeople50_statements, maxlen=config['MAX_QPAIRS'])
